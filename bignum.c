...
#define CHECKBIG1 if(big_pt1<0 || big_pt1>=BIGSIZE){exception(RESOURCE_ERR,makestr("bigcell pt1"),big_pt1,0);}
...
int bigx_div1(int arg1, int arg2)
{
	...
	subtract =
	    bigx_shift(bigx_mult1(arg2, bigx_int_to_big(makeint(q))),
		       shift);
...
int bigx_ntt_mult(int x, int y)
{
	...
	exception(RESOURCE_ERR, NIL, makestr("ntt-mult"), 0);
