...
int exact_to_inexact(int x){
    ...
    switch(tag){
        case INTN:  res = freshcell();
                    ...
                    SET_FLT(res,val);
                    return(res);
        case LONGN:  res = freshcell();
                    SET_TAG(res,FLTN);
                    val = (double)GET_LONG(x);
                    SET_FLT(res,val);
                    return(res);
        case BIGX:  return(bigx_big_to_flt(x));
        case FLTN:  return(x);
        default:    return(x);
    }
    ...
    switch(tag1){
        case INTN:
            switch(tag2){
                ...
                case LONGN: return(bigx_minus(bigx_int_to_big(arg1),
        ...
        case LONGN:
            switch(tag2){
                ...
                case FLTN:  return(minus(exact_to_inexact(arg1),arg2));
    ...
    switch(tag1){
        case INTN:
            switch(tag2){
                ...
                case LONGN:  if(GET_INT(arg1) != 0)
                                return(bigx_mult(bigx_int_to_big(arg1),bigx_long_to_big(arg2)));
                            else
                                return(arg1); //int 0
        ...
        case LONGN:
            switch(tag2){
                case INTN:  if(GET_INT(arg2) != 0)
                                return(bigx_mult(bigx_long_to_big(arg1),bigx_int_to_big(arg2)));
                            else
                                return(arg2); //int 0
                case FLTN:  return(mult(exact_to_inexact(arg1),arg2));
...
int angle(int y, int x){
    ...
    if(positive_zerop(y) && positivep(x))
        ...
        return(makeflt(atan(GET_FLT(exact_to_inexact(y)) / GET_FLT(exact_to_inexact(x)))));
    else if(positivep(y) && zerop(x))
        return(makeflt(PI / 2));
    else if(positivep(y) && negativep(x))
        return(makeflt(PI + atan(GET_FLT(exact_to_inexact(y)) / GET_FLT(exact_to_inexact(x)))));
...
int gcd(int x, int y){
    ...
    if(integerp(x) && integerp(y))
        ...
        return(makeflt((double)abs(int_gcd((int)GET_FLT(x),GET_INT(y)))));
    
    else if(integerp(x) && floatp(y))
        return(makeflt((double)abs(int_gcd(GET_INT(x),(int)GET_FLT(y)))));
...
int lcm(int x, int y){
    int g,d,res;
    if(integerp(x) && integerp(y) &&
        abs(GET_INT(x)) < 10000 && abs(GET_INT(y)) < 10000)// �Ȃ��Ȃ�x,y < sqrt(BIGNUM_BASE)
        return(makeint(abs(int_lcm(GET_INT(x),GET_INT(y)))));
    
    else if(floatp(x) && integerp(y))
        return(makeflt((double)abs(int_lcm((int)GET_FLT(x),GET_INT(y)))));
    
    else if(integerp(x) && floatp(y))
        return(makeflt((double)abs(int_lcm(GET_INT(x),(int)GET_FLT(y)))));
    
    else if(floatp(x) && floatp(y))
        return(makeflt((double)abs(int_lcm(GET_FLT(x),(int)GET_FLT(y)))));
