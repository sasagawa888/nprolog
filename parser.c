...
int parser(int operand, int operator, int weight, int spec, int terminal, int parsemode){
    ...
    if(terminal == 1 &&
            ...
            stok.type == RPAREN ||
            stok.type == RCURL ||
            stok.type == VERTICAL)){ //these are terminations
        #if CHECK
        ...
        (stok.ch == '(' || stok.ch == NUL) && stok.ahead != ' ' &&
         GET_OPT(temp) != FX && GET_OPT(temp) != FY){
            ...
            if(stok.type == LPAREN){
                ...
                temp2 = parser(cons(temp1,operand),operator,weight,spec,terminal,parsemode);
                prefix_flag = 1;
                return(temp2);
            }
            else{
    ...
    if(infix_operator_p(temp) &&
        ...
        if(nullp(operator)){
            ...
            if(w1 == 1200){
                ...
                return(parser(cons(list2(temp,temp1),operand),operator,weight,spec,terminal,parsemode));
            }
        }
        else if(w1 < w2){
            #if CHECK
        ...
        return(parser(cons(temp2,operand),operator,weight,spec,terminal,parsemode));
    }
    else if(infix_operator_p(temp) &&
            (GET_OPT(temp) == XFX ||
             GET_OPT(temp) == FX_XFX ||
        ...
        if(nullp(operator)){
            ...
            printf("null operator\n");
            #endif
            return(parser(operand,cons(temp,operator),weight,spec,terminal,parsemode));
        }
        else if(w1 == w2){
            #if CHECK
            printf("w1 == w2\n");
            #endif
            temp1 = list3(temp,car(operand),parser(NIL,NIL,w1,XFX,1,parsemode));
            return(parser(cons(temp1,cdr(operand)),operator,weight,spec,terminal,parsemode));
        }
        else if(w1 < w2){
            #if CHECK
            printf("w1 < w2\n");
            ...
            if(stok.type == RPAREN){
                ...
                temp2 = list3(temp,car(operand),temp1);
                return(parser(cons(temp2,cdr(operand)),operator,weight,spec,terminal,parsemode));
            }
        }
        else{
            #if CHECK
            ...
            return(parser(list1(temp1),cons(temp,cdr(operator)),weight,spec,terminal,parsemode));
        }
    }
    else if(infix_operator_p(temp) &&
            (GET_OPT(temp) == YFX ||
        ...
        if(nullp(operator)){
            ...
            printf("null operator\n");
            #endif
            temp1 = parser(operand,cons(temp,operator),w1,YFX,1,parsemode);
            return(parser(list1(temp1),NIL,weight,spec,terminal,parsemode));
        }
        else if(w1 == w2){
            #if CHECK
            printf("s1 == w2\n");
            ...
            temp2 = list3(temp,cons(car(operator),reverse(operand)),temp1);
            return(parser(list1(temp2),NIL,weight,spec,terminal,parsemode));
        }
        else if(w1 < w2){
            #if CHECK
            printf("s1 < w2\n");
        ...
        if(nullp(operator)){
            ...
            printf("w1 == w2\n");
            #endif
            temp1 = parser(list1(car(operand)),list1(temp),w1+1,XFY,1,parsemode);
            ...
            #endif
            temp1 = cons(car(operator),reverse(operand));
            temp2 = parser(list1(temp1),list1(temp),w1+1,XFY,1,parsemode);
            return(parser(list1(temp2),cdr(operator),weight,spec,terminal,parsemode));
        }
    }
    else if(infix_operator_p(temp) &&
            (GET_OPT(temp) == XF ||
        ...
        if(nullp(operator)){
            ...
            return(parser(cons(temp1,NIL),operator,weight,spec,terminal,parsemode));
        }
        else if(w1 < w2){
            #if CHECK
            printf("w1 < w2\n");
            ...
            temp1 = cons(car(operator),reverse(operand));
            temp2 = list2(temp,temp1);
            return(parser(cons(temp2,NIL),NIL,weight,spec,terminal,parsemode));
        }
    }
    else if(infix_operator_p(temp) &&
            (GET_OPT(temp) == YF ||
...
char str[256], *e;
    ...
    switch(c){
        case '(':   stok.type = LPAREN; return;
        case ')':   stok.type = RPAREN; return;
        case '[':   stok.type = LBRACKET; return;
        case ']':   stok.type = RBRACKET; return;
        case '|':   stok.type = VERTICAL; return;
        case '{':   c1 = readc();
                    if(c1 == '}'){
                        stok.buf[0] = '{';
                        stok.buf[1] = '}';
                    ...
                    unreadc(c1);
                    stok.type = LCURL; return;
        case '}':   stok.type = RCURL; return;
        case ',':   stok.type = COMMA; return;
        case ';':   stok.type = SEMICOLON; return;
        case '.':   c = readc();
                    if(c <= ' '){
                        stok.type = PERIOD;
                        unreadc(c);
...
int readitem(void){
    ...
    switch(stok.type){
        ...
        case COMPILED:  temp = makeatom(stok.buf,COMP);
                        gettoken();
                        if(stok.type == LPAREN){
                            if(stok.space == SKIP)
        ...
        case VERTICAL:  return(makeatom("|",SIMP));
        ...
        case VARIABLE:  temp = makeatom(stok.buf,VAR);
