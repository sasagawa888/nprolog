...
# 頭がオーバーヒート
このところずっとCLPFDについて考え続けています。わかったような気がして
コードを書いてみると、いやいやまだまだ理解が浅かった。再び考え直し、これを
繰り返しています。CHATGPTにはお世話になります。鍵は以下に探索範囲を狭めて
効率よく解を見つけるかということは身に染みました。
...
このとき処理系はどういう解析をするのか？鉛筆とノートで追いかけてみました。
...
all_differentを使うと変数ごとに異なる解を得ることができます。
これは解を推定しつつ他の制約条件を満たすがどうかをあわせて判定します。
このため8Queens問題において効率的です。通常のPrologの場合ですと
明示的に枝刈りをコードで示さないといけません。しかし、CLPFDの場合には
これを自動的に行いつつ解空間を狭めて効率を図ります。
