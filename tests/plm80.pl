...
plm80(File) :-
    ...
    open(S,Out,w),
    format(S,$INCLUDE ~A$,['plmbase.asm']),nl(S),
    output_asm(S,Asm),
    close(S).
...
% ----------------------------------------
% output_asm(+Stream, +Lines)
% Lines is a list of label/1 or instr/2 terms.
% ----------------------------------------
...
% ----------------------------------------
% normalize_args(+ArgsOrArg, -ArgsList)
% ----------------------------------------
...
% ----------------------------------------
% output_operands(+Stream, +Operands)
% ----------------------------------------
