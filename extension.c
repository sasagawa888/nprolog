...
int arg1,arg2,arg3,varYs,varY,varZs,varX,varXs,n,body,save1,save2,res;
save2 = sp[th];
n = length(arglist);
if(n == 3){
arg1 = car(arglist);
...
save1 = get_wp(th);
if(unify_var(varX,arg1,th) == YES && unify(wlistcons(varX,varXs,th),arg2,th) == YES && unify_var(varXs,arg3,th) == YES)
...
save1 = wp[th];
if(unify_var(varX,arg1,th) == YES && unify(wlistcons(varY,varYs,th),arg2,th) == YES && unify(wlistcons(varY,varZs,th),arg3,th) == YES)
{body = wcons(makesys("select"),wcons(varX,wcons(varYs,wcons(varZs,NIL,th),th),th),th);
if((res=prove_all(addtail_body(rest,body,th),sp[th],th)) == YES)
return(YES);
unbind(save2,th);
set_wp(save1,th);}
return(NO);}
